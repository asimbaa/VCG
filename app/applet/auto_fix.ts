...
while (true) {
    try {
        ...
        } else if (msg.includes("Unexpected closing fragment tag does not match opening \"div\" tag")) {
            // Add one </div> before </>
            let matchIndex = code.lastIndexOf('</>');
            if (matchIndex === -1) break;
            code = code.substring(0, matchIndex) + '</div>\n                  ' + code.substring(matchIndex);
        } else if (msg.includes("JSX element 'div' has no corresponding closing tag")) {
            // Unclosed div globally. 
